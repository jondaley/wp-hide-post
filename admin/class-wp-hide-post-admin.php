 ...
 */
if (!class_exists('wp_hide_post_Admin'))
{
	class wp_hide_post_Admin
	{
		...
		public function __construct($wp_hide_post, $version)
		{
			...
			$this->post_types = apply_filters('wphpp_available_post_types', array('post', 'page'), 'selected');
			if (getenv('running_test'))
			{
		...
		public function plugin_init1()
		{
			...
			foreach ($this->allowedPostTypes() as $post_type)
			{
				...
				{
					wp_hide_post::getInstance()->get_loader()->add_action("manage_pages_custom_column", $this->plugin_admin, 'manage_posts_columns', 10);
				}
				else
				{
					wp_hide_post::getInstance()->get_loader()->add_action("manage_{$post_type}_posts_columns", $this->plugin_admin, 'manage_posts_columns', 10);
					wp_hide_post::getInstance()->get_loader()->add_action("manage_edit-{$post_type}_columns", $this->plugin_admin, 'manage_posts_columns', 10);
				}
			}
		}
		/**
		...
		*/
		public function get_post_visibility_types()
		{
			...
			$this->post_visibility_types = array(
				...
				'no_index' => array(
					'label' => wphp_('Hide in Search engine'),
					'short_label' => 'Search engine',
					'description' => 'Hides in Search engines like (Google, Yahoo ,Bing etc.) <br/>If it is an old post and you are hiding it first time make sure to notify search engine about this ,<br/> For exaample in case of google you can visit <a href="https://www.google.com/webmasters/tools/googlebot-fetch?pli=1" target="_blank">this</a> page and request ',
					'no_auto_join' => true,
				),
				'post_rel' => array(
		...
		*/
		public function get_page_visibility_types()
		{
			...
			$this->page_visibility_types = array(
				...
				'hide_always' => array(
					'label' => wphp_('Hide everywhere(not in search engines) pages are listed. '),
					'short_label' => 'Always',
					'description' => 'Will still show up in sitemap.xml if you generate one automatically',

				),
				'nohide_search' => array(
					'label' => wphp_('Hide everywhere(not in search engines) but keep in search results.'),
				...
				'no_index' => array(
					'label' => wphp_('Hide in Search engine'),
					'short_label' => 'Search engine',
					'description' => 'Hides in Search engines like (Google, Yahoo ,Bing etc.) <br/>If it is an old page and you are hiding it first time make sure to notify search engine about this ,<br/> For exaample in case of google you can visit <a href="https://www.google.com/webmasters/tools/googlebot-fetch?pli=1" target="_blank">this</a> page and request ',
					'no_auto_join' => true,
				),
			);
		...
		{
			// p_l($_POST);
			if (empty($_POST[WPHP_VISIBILITY_NAME]))
			{
				//  return;
			}
			if (!defined('WP_TESTS_DOMAIN') && (!$this->allowedPostTypes($post_type) || !($can = current_user_can($post_type == 'page' ? 'edit_page' : 'edit_post', $post_id))))
			{
				p_l(" save_visibility $post_type allowedPostTypes=".((int) $this->allowedPostTypes($post_type)).",current_user_can=".(int) $can);

				return;
			}
			p_l(" save_visibility  bulk=".($is_bulk ? 1 : 0));

			p_l(WPHP_VISIBILITY_NAME);

			$visibility_types = $this->get_visibility_type_values($post_type);
			foreach ($visibility_types as $index => $v)
			{
				if (!isset($_POST[WPHP_VISIBILITY_NAME][$index]))
				{
			...
			return;
			if (!isset($_POST[WPHP_VISIBILITY_NAME."_old"]) || (isset($_POST[WPHP_VISIBILITY_NAME."_old"]) && !is_array(isset($_POST[WPHP_VISIBILITY_NAME."_old"]))))
			{
				if (isset($_POST[WPHP_VISIBILITY_NAME]) && is_array($_POST[WPHP_VISIBILITY_NAME]))
				{
					foreach ($_POST[WPHP_VISIBILITY_NAME] as $index => $value)
					{
						$_POST[WPHP_VISIBILITY_NAME."_old"][$index] = "";
					}
				}
			}
		...
		{
			p_l("update_visibility $id, $lp_flag, $lp_value");
			if ($lp_value)
			{
 ...
 */
		...
		{
			p_l("called: wphp_delete_post $post_id");
			global $wpdb;
...
// return the visibility options set for a page or post
		...
		public function get_visibility_type_values($post_type, $post_id = 0)
		{
			global $wpdb;
			$post_id = (int) $post_id;
			...
			$sql = "SELECT meta_key from ".WPHP_TABLE_NAME." where post_id = $post_id and meta_key like '".WPHP_META_VALUE_PREFIX."%'";
			$flags = $wpdb->get_results($sql, ARRAY_N);
			p_l($sql);
			if ($flags)
			{
...
// display data of our custom column
		...
		public function display_custom_quickedit($column_name, $post_type)
		{
			...
			$html = '<div style="padding:10px;padding-bottom:0px"><a target="_blank"  style="color:red" href="http://scriptburn.com/wphp">WP hide post Quick edit mode is for Pro Users only</a></div>';
			$html = apply_filters('wphpp_quick_edit', $html, $this, $post_type);
...
// Filter the posts acording to selected filter in post list select box
		...
		{
			return apply_filters('wphpp_default_info', $this->default_footer(0), $page);
		}
...
<div>
    <div style="float:left">
        <a href="https://scriptburn.com/wp-hide-post/#comments">
            %1$s
        </a>
    </div>
    <div style="float:right">
        <a href="https://wordpress.org/extend/plugins/wp-hide-post/">
            %2$s
        </a>
    </div>
</div><div style="clear:both">'.($price ? '' : '
<div style="text-align:center">
    <a href="https://scriptburn.com/wp-hide-post">
...
');
		...
		{
			$id = 'supress_activate_'.(!file_exists(WPHP_PLUGIN_DIR.'admin/license/autoload.php') ? 'pro' : 'lite');

			if (isset($_GET[$id]))
			{
				if ($id == 'supress_activate_pro')
				{
					echo '<div class="error"><p>Please disable WP Hide Post Plugin first to activate Pro version</p></div>';
				}
				else
				{
					echo '<div class="error"><p>Please disable WP Hide Post Pro Plugin first to activate Lite version</p></div>';
				}
			}
		}
	}
